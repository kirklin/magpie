...
export function SearchBar({ value, onChange, onCompositionChange, activeFilter, onFilterChange, placeholder, ref }: SearchBarProps & { ref?: React.Ref<SearchBarRef> }) {
  const t = useT();
  ...
  // When dropdown opens, highlight the currently active filter. Derived during
  // render rather than in an effect so the first painted frame already shows
  ...
  useEffect(() => {
    if (isDropdownOpen && highlightedIndex >= 0) {
      itemElementsRef.current.get(highlightedIndex)?.scrollIntoView({ block: "nearest" });
    }
